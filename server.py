...
#    xmlrpc_db.help = "use binary to echo things back safely"
    ...
    def checkin(self, transaction, username, place, now):
        user = transaction.execute("select * from users where username=%s", (username, ))
        result = transaction.fetchall()
        print result
        updated = False
        if len(result):
            transaction.execute("update users set place=%s,last_checkin=%s where username=%s", (place, now, username))
            updated=True
        else:
            transaction.execute("insert into users (username, place, last_checkin)values(%s, %s, %s)", (username, place, now))
        transaction.execute("select * from users where username=%s", (username, ))
...
def initTables(transaction):
    ...
    if not len(result):
        ...
        for username,place,last_checkin in default_users:
            transaction.execute("INSERT INTO `users` (username, place,last_checkin) VALUES(%s, %s, %s)", (username, place,last_checkin))
if __name__ == '__main__':
    ...
    for k in ['host', 'user', 'passwd', 'db']:
        mysql_config[k]=config.get('db', k)
